...
from asr_client import ASRClient, LocalASRClient
...
def process_video(
    ...
    print(f"\n====== [DEBUG] process_video 函数被调用 ======")
    print(f"参数 video_path: {video_path}")
    print(f"参数 asr_server_url: {asr_server_url}")
    """
    处理视频生成笔记的主函数
    ...
    try:
        ...
        if asr_mode == '本地模式 (whisper.cpp)':
            ...
            if not whisper_cli_path or not model_path:
                raise RuntimeError(f"本地模式需要配置 whisper_cli_path 和 model_path，请检查 config.yaml")
            
            asr_client = LocalASRClient(whisper_cli_path, model_path)
            
            # 本地模式检查：验证文件是否存在
            if not asr_client.check_health():
                missing_files = []
                if not os.path.exists(whisper_cli_path):
                    missing_files.append(f"whisper-cli可执行文件: {whisper_cli_path}")
                if not os.path.exists(model_path):
                    missing_files.append(f"模型文件: {model_path}")
                
                error_msg = f"本地ASR文件缺失:\n" + "\n".join(f"  - {f}" for f in missing_files)
                error_msg += f"\n\n请确保：\n1. whisper-cli路径正确且文件存在\n2. 模型文件已下载到指定位置\n3. 在【配置设置】中检查路径配置"
            ...
            logger.info(f"ASR客户端初始化成功，模式: {asr_mode}，客户端类型: {type(asr_client).__name__}")
        ...
        asr_result = asr_client.recognize_audio(audio_path)
        if asr_result is None:
            raise RuntimeError("语音识别失败，返回None")
        logger.info(f"语音识别成功")

        # 修复：从 asr_result 中获取结构化段落
        segments = asr_result.get("segments", [])
        if not segments:
            raise RuntimeError("语音识别失败：未解析出任何段落")
            
        # 如果需要将段落拼接成完整文本
        transcript = "\n".join([f"[{seg['start']} --> {seg['end']}] {seg['text']}" for seg in segments])
        # 或者直接使用完整文本
        transcript = asr_result.get("full_text", "")
            
        result["transcript"] = transcript
        result["segments"] = segments  # 保存结构化段落，可能对后续处理有用
