🚀 **本仓库是 [Marstaos/MetaNote](https://github.com/Marstaos/MetaNote) 的衍生版本，主要增强了本地语音识别、长视频支持等特性。感谢原作者的杰出工作！**
...
- **🚀 本地 ASR 引擎**：将云端语音识别替换为 [whisper.cpp](https://github.com/ggerganov/whisper.cpp)，实现完全离线转录，并针对 Apple M 系列芯片（如 M4）进行 GPU 加速，处理速度提升明显。
- **📝 超长视频支持**：引入智能分段笔记生成机制，自动将长视频转录文本切分为多个片段，分别调用大模型生成笔记后再合并，彻底避免 API Token 超限问题，完整保留所有内容。
- **🛠️ 界面与稳定性修复**：
  - 修复 Streamlit 界面因函数返回值缺失导致的 `TypeError` 崩溃。
  - 去除 whisper.cpp 输出中的 ANSI 颜色字符，避免笔记中出现乱码。
- **🎯 路径配置优化**：将配置文件中的关键路径（如模型路径、输出目录）改为相对路径，使整个项目文件夹可以随意移动而无需重新配置。
- **🎵 音频文件支持**：提供 `convert_audio_to_metanote.sh` 脚本，可将纯音频（MP3、M4A 等）快速转换为带静态画面的 MP4，无缝接入视频处理流程。
