...
class QwenNoteGenerator(NoteGenerator):
    ...
    def generate_notes(
        ...
        # 定义每段最大字符数（保守估计，确保不超过 API token 限制）
        max_chars_per_segment = 10000  # 可根据需要调整

        # 如果文本不长，直接生成
        if len(transcript) <= max_chars_per_segment:
            return self._generate_single_notes(transcript, frames_text, frames_info, output_path, title)
        else:
            # 分段生成
            logger.info(f"转录文本过长 ({len(transcript)} 字符)，将分为多段生成笔记")
            segments = self._split_transcript_by_chars(transcript, max_chars_per_segment)
            notes_parts = []
            for idx, segment in enumerate(segments):
                logger.info(f"正在生成第 {idx+1}/{len(segments)} 部分笔记...")
                part_notes = self._generate_single_notes(
                    segment, 
                    frames_text,   # 每段都包含所有帧信息（简化）
                    frames_info, 
                    None,          # 不单独保存文件
                    f"{title} 第{idx+1}部分" if title else f"第{idx+1}部分"
                )
                notes_parts.append(part_notes)
            # 合并所有部分
            final_notes = self._merge_notes_parts(notes_parts, title, frames_info)
            # 保存到文件
    ...
    def _split_transcript_by_chars(self, text: str, max_chars: int) -> List[str]:
    # 先按双换行（自然段落）分割
        paragraphs = text.split('\n\n')
        
        # 如果只有一个段落但总长度超过限制，则按行分割（每行是时间戳+文本）
    ...
    def _generate_single_notes(self, transcript_part: str, frames_text: str, frames_info: List[Dict], output_path: Optional[str], title: str) -> str:
        """
        为单个片段生成笔记（返回内容字符串，不保存）
        """
...
请直接给出完整的Markdown文本，不要解释你的处理过程。确保生成的笔记内容全面、系统，能够作为学习参考资料使用。"""
        ...
        try:
            ...
            logger.info(f"片段笔记生成完成，耗时：{time.time()-start_time:.2f}秒")
            return generated
        except Exception as e:
            logger.error(f"生成片段笔记时出错: {str(e)}")
            return self._generate_basic_notes_for_segment(transcript_part, frames_info, title)

    # ----- 新增方法3：片段后备基本笔记 -----
    def _generate_basic_notes_for_segment(self, transcript: str, frames_info: List[Dict], title: str) -> str:
        """当API调用失败时，直接返回原始转录文本作为该片段笔记"""
        return f"## {title}\n\n{transcript}\n\n"

    # ----- 新增方法4：合并所有片段笔记 -----
    def _merge_notes_parts(self, parts: List[str], title: str, frames_info: List[Dict]) -> str:
        merged = f"# {title}\n\n"
        merged += "\n\n---\n\n".join(parts)
        merged += f"\n\n---\n\n*这些笔记由MetaNote自动生成于 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*"
        return merged
